...
def test_extraction(format_name="gen9ou"):
    ...
    usage_data_full = process_stats.load_data(stats_file_path)
    ...
    print("\n--- Usage Stats ---")
    print(json.dumps(process_stats.extract_usage_stats(usage_data, top_pokemon), indent=2))
    
    print("\n--- Base Stats ---")
    print(process_stats.extract_base_stats(top_pokemon, pokedex))
    
    print("\n--- Types ---")
    print(process_stats.extract_types(top_pokemon, pokedex))
    
    print("\n--- Top 5 Moves ---")
    moves_data = process_stats.extract_moves(usage_data, top_pokemon, moves)
    print(json.dumps(moves_data[:5], indent=2))
    
    print("\n--- Top 5 Teammates ---")
    teammates_data = process_stats.extract_teammates(usage_data, top_pokemon)
    print(json.dumps(teammates_data[:5], indent=2))
    
    print("\n--- Top 5 Items ---")
    items_data = process_stats.extract_items(usage_data, top_pokemon, items)
    print(json.dumps(items_data[:5], indent=2))
    
    print("\n--- Top 5 Abilities ---")
    abilities_data = process_stats.extract_abilities(usage_data, top_pokemon, abilities)
    print(json.dumps(abilities_data[:5], indent=2))
    
    print("\n--- Top 5 Natures ---")
    natures_data = process_stats.extract_natures(usage_data, top_pokemon)
    print(json.dumps(natures_data[:5], indent=2))
    
    print("\n--- Top 5 Spreads ---")
    spreads_data = process_stats.extract_spreads(usage_data, top_pokemon)
    print(json.dumps(spreads_data[:5], indent=2))
    
    print("\n--- EVs Summary (Top 1 per category) ---")
    evs_data = process_stats.extract_evs(usage_data, top_pokemon)
    summary_evs = {k: v[:1] for k, v in evs_data.items()}
    print(json.dumps(summary_evs, indent=2))
    
    print("\n--- Top 5 Tera Types ---")
    tera_data = process_stats.extract_tera_types(usage_data, top_pokemon)
    print(json.dumps(tera_data[:5], indent=2))
    
    print("\n--- Top 5 Checks and Counters ---")
    counters_data = process_stats.extract_checks_and_counters(usage_data, top_pokemon)
